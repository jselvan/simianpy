...
@click.option('-o','--output','output', default='out.nex', help='Output filepath. Default=out.nex')
@click.option('-?','--whatif', 'whatif', default=False, help='Prompts user before any actions are taken', is_flag=True)
...
def Combine(whatif, force, inputs, output):
    ...
    if output.is_file() and not force:
        raise FileExistsError(f"A nex file named {output} already exists. Specify a different output file name or use the force parameter to overwrite.")
    ...
    with simi.io.Nex(output, mode='w', timestampFrequency=max_timestampFrequency) as out_file:
