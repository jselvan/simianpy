...
def GaussianKDE2DPlot(x, y, weights, data=None, range=None, xticks=None, yticks=None, resolution=100, bw_method=None, norm=True, ax=None, im_kwargs=dict(), return_density=False):
    if data is None:
        if weights is None:
    ...
    density = pdf.evaluate(range=range,xticks=xticks,yticks=yticks,resolution=resolution,return_series=True)
